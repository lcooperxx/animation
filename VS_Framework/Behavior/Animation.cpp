...
void Joint::UpdateTransformation(bool bRecursive)
{
	...
	m_global.Identity();
	if (m_pParent == NULL)//判断m_global的父结点是否为空
	{
		m_global = m_local;//空的话，m_global = m_local
	}
	else
	{
		m_global = m_pParent->m_global * m_local;//否则用公式表达m_global
	}
	if (bRecursive)
	{
		vector<Joint*>::const_iterator iter;
...
{
	if (pRoot == NULL)//如果pRoot为空，就从骨架的根节点更新m_pRoot
	{
		...
		if (dist.Length() > EPSILONDIST) // There is need for CCD
		{
			...
			for (unsigned int i = 0; i < MAX_ITER; i++)
			{
				...
				for (vector<Joint*>::const_iterator iter = m_ikChain.begin(); iter != m_ikChain.end(); ++iter)
				{
					...
					// Add your code here
					vec3 dir_cur, dir_target;
					dir_cur = pEndEffector->GetGlobalTranslation() - pJoint->GetGlobalTranslation();
					dir_target = m_goalPosition;
					float angle;
					vec3 axis;
					...
					angle = acosf(dir_cur * dir_target);
					axis = dir_cur.Cross(dir_target);

					mat3 mat3;
					mat3 = mat3.Rotation3DRad(axis, angle);
					mat3 = mat3 * pJoint->GetLocalRotation();
					pJoint->SetLocalRotation(mat3);
					// Update rotation at pJoint
					// Update FK from pJoint
					// Add your code
					pJoint->UpdateTransformation();
					(*this).UpdateFK();
...
void Player::Blend( const unsigned int& startFrame, const unsigned int& endFrame, const unsigned int& interpFrame, const unsigned int& type )
{
	...
	unsigned int interFrame = 0;
	totalFrame = startFrame + m_motion2.GetFrameCount() - endFrame;
    ...
    // m_motion3[0, ..., startFrame] = m_motion1[0, ..., startFrame]//把1给三
    for (unsigned int i = 0; i < startFrame; i++)
    {
        m_motion3.m_keyFrames[i]->Clone(*(m_motion1.m_keyFrames[i]));
    }

    // Estimate root translation and rotation f  or m_motion3[startFrame + interpFrame + 1]
    ...
    // TODO: Start your code here...//3的方向，位置
	float angle;
	vec3 dir1, dir2;
	dir1 = m_motion1.m_keyFrames[startFrame-1]->GetRootTranslation() - m_motion1.m_keyFrames[startFrame]->GetRootTranslation();
	dir2 = m_motion2.m_keyFrames[endFrame]->GetRootTranslation() - m_motion2.m_keyFrames[endFrame + 1]->GetRootTranslation();

	dir1.Normalize(); 
	dir2.Normalize();
	
	angle = acosf(dir1 * dir2);
	mat3 mat3;
	mat3 = mat3.Rotation3DRad(vec3(0, 1, 0), -angle);
	...
	for (unsigned int i = startFrame, j=0; i < totalFrame; i++,j++)
	{
		vec3 t1, t2;
		t1 = m_motion2.m_keyFrames[endFrame + j]->GetRootTranslation() - m_motion2.m_keyFrames[endFrame - 1]->GetRootTranslation();
		t2 = mat3 * t1;

		m_motion3.m_keyFrames[i]->Clone(*(m_motion2.m_keyFrames[endFrame + j]));
		m_motion3.m_keyFrames[i]->SetJointRotation(0, mat3 * m_motion3.m_keyFrames[i]->GetJointRotation(0));

		m_motion3.m_keyFrames[i]->SetRootTranslation(m_motion1.m_keyFrames[startFrame-1]->GetRootTranslation() + t2);
 ...
 //   Frame *pFrame;
	//int i, j;
 //   for (i = startFrame + 1, j = 0; i < startFrame + 1; i++, j++)
 //   {
